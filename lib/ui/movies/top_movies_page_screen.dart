...
import 'package:movieapp/bloc/top_movies_query_bloc.dart';
import 'package:movieapp/datalayer/movie.dart';
import 'package:movieapp/ui/movie_tile.dart';

// ignore: must_be_immutable
class TopMoviesPageScreen extends StatelessWidget {
  ...
  TopMoviesPageScreen({Key key, @required this.scrollController})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    final bloc = TopMovieQueryBloc();

    if (currentPage == 1) loadPage(bloc, currentPage);

    return BlocProvider<TopMovieQueryBloc>(
      bloc: bloc,
      child: Scaffold(
  ...
  void loadPage(TopMovieQueryBloc bloc, int page) {
    bloc.getTopMovies(page);
  }

  Widget _buildResults(TopMovieQueryBloc bloc) {
